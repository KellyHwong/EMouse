...
void PID_Move(uint8_t l_Dir, uint8_t r_Dir, float l_RPS,  float r_RPS)
{
    ...
    if(r_RPS<0) r_RPS=0;
    //限幅，不要超过MAX_RPS
    if(l_RPS>MAX_RPS) l_RPS=MAX_RPS;
    if(r_RPS>MAX_RPS) r_RPS=MAX_RPS;
    //
    g_L_Dir = l_Dir;
    g_L_RPS = l_RPS;
    g_R_Dir = r_Dir;
...
//        if (diff_rps>MAX_RPS_DIFF) diff_rps=MAX_RPS_DIFF;
    ...
    //        }
                ...
                if (g_L_Sample_RPS>5) {
                    success_count++;
                    if (success_count>=1000) success_flag = 1;
                    if (success_flag){
...
void PID_Check_Timeout(void)
{
    ...
    //1S后停止电机
//    if (PID_Ticks>=PID_TIMEOUT_TICKS){
//        PID_Stop();
