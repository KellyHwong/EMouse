...
void Motor_Check_Timeout(void) {
  ...
  if (Motor_G && (Motor_Accel_Ticks <= ACCEL_TIMEOUT_TICKS)) {
    ...
    tmpL = g_L_Cur_PWM;
    tmpR = g_R_Cur_PWM;
    Motor_Move(g_L_Cur_Dir, g_R_Cur_Dir, (uint16_t) l_PWM_Div,
               (uint16_t) r_PWM_Div);
    g_L_Cur_PWM = tmpL;
    g_R_Cur_PWM = tmpR;
  }
  //1S后以100mS间隔采集数据
  if (Motor_G && (Motor_Ticks >= 100)) {
    if (0 == (Motor_Ticks % 10)) {
      if (dataPtr < DATA_NUM) {
        data[0][dataPtr] = g_L_RPS;
        data[1][dataPtr] = g_R_RPS;
        data[2][dataPtr] = g_L_Sample_RPS;
        data[3][dataPtr] = g_R_Sample_RPS;
        data[4][dataPtr] = (float) g_L_Cur_PWM;
        data[5][dataPtr] = (float) g_R_Cur_PWM;
        dataPtr++;
      }
    }
  }
...
void Motor_Print_Data(void) {
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < DATA_NUM; j++) {
      UARTprintf("%d", (uint32_t) (100 * data[i][j]));
      if ((DATA_NUM - 1) == j)
        UARTprintf("\n");
      else
        UARTprintf(" ");
  ...
  for (i = 4; i < 6; i++) {
    for (j = 0; j < DATA_NUM; j++) {
      UARTprintf("%d", (uint32_t) (data[i][j]));
      if ((DATA_NUM - 1) == j)
        UARTprintf("\n");
      else
        UARTprintf(" ");
...
inline void Motor_Sample_RPS(void) {
  Motor_RPS_Sampled = 1;
  g_L_Sample_RPS = QEIVelocityGet(LEFT_QEI) / LINES / LINE_DIV * 1000.0
      / QEI_UPDATE_TIME * SMALL_WHEEL / BIG_WHEEL;
  g_R_Sample_RPS = RATE * QEIVelocityGet(RIGHT_QEI) / LINES / LINE_DIV * 1000.0
      / QEI_UPDATE_TIME * SMALL_WHEEL / BIG_WHEEL;
}
...
//改变当前PWM脉宽，启动电机
//可以控制电机方向
void Motor_Move(uint8_t leftCtl, uint8_t rightCtl, uint16_t leftWidth,
                uint16_t rightWidth) {
  Motor_G = 1;
  g_L_Cur_PWM = leftWidth;
  g_R_Cur_PWM = rightWidth;
...
//QEI速度配置
  ...
  QEI_UPDATE_TIME * SysCtlClockGet() / 1000);
  QEIVelocityConfigure(QEI1_BASE, QEI_VELDIV_1,
  QEI_UPDATE_TIME * SysCtlClockGet() / 1000);
//初始化QEI位移为0
